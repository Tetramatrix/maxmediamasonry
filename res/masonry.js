        ...
        if (json && json.length)
        {
          ...
          $j.each(json, function(idx, ele)
          {
            if (ele.killbit == "on")
            {
              ...
              {
                container.append($j("#smilieTemplate").tmpl(ele).css({
                "display": "block"
                })).masonry('reload');
              ...
              {
                container.prepend($j("#smilieTemplate").tmpl(ele).css({
                  "display": "block"
                })).masonry('reload');
              }
            } else
      ...
      if (response && response.length)
      {
        ...
        $j.each(response, function(idx, ele)
        {
          if (ele.Isimage == "true")
          {
            ...
            container.prepend($j("#smilieTemplate").tmpl(ele)).masonry('reload');
          }
          container.imagesLoaded(function()
          {
            ++counter;
...
$(document).ready(function() {
  $("#container").draggable({zIndex:-100});
